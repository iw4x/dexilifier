...
float4 var_shadowmapspotC = tex2Dproj(shadowmapSamplerSpot, var_I);
	...
	var_shadowmapspotC.gba = lightPosition.xyz + (-inputVx.texcoord5);
	var_shadowmapspotB.x = dot(var_shadowmapspotC.gba, var_shadowmapspotC.gba);
	var_shadowmapspotB.x = rsqrt(var_shadowmapspotB.x);
	var_shadowmapspotC.gba = var_shadowmapspotC.gba * var_shadowmapspotB.xxx;
	var_shadowmapspotB.x = 1 / var_shadowmapspotB.x;
	var_shadowmapspotB.y = dot(var_shadowmapspotC.gba, lightSpotDir.xyz);
...
half var_M = lerp(var_shadowmapspotC.r, var_lightmapprimary.x, var_shadowmapspotB.y);
	var_shadowmapspotC.r = saturate(var_shadowmapspotB.x * lightPosition.w);
	var_shadowmapspotB = var_shadowmapspotC.r * lightFalloffPlacement.xy + lightFalloffPlacement.zw;
...
float4 var_P;
var_P.x = var_lightmapsecondaryA.w;
...
half4 var_lightmapsecondaryB = tex2D(lightmapSamplerSecondary, var_P.zw);
	...
	var_shadowmapspotC.r = dot(var_P.xy, var_P.xy) + 1;
	var_shadowmapspotC.r = saturate(rsqrt(var_shadowmapspotC.r));
	var_lightmapsecondaryA.xyz = var_lightmapsecondaryB.xyz * var_shadowmapspotC.r + var_lightmapsecondaryA.xyz;
	...
	var_shadowmapspotC.r = saturate(dot(var_shadowmapspotC.gba, var_P.xyz));
	var_shadowmapspotC.rgb *= lightDiffuse.xyz;
	var_shadowmapspotC.rgb = var_lightmapsecondary * var_shadowmapspotC.rgb + var_lightmapsecondaryA.xyz;
