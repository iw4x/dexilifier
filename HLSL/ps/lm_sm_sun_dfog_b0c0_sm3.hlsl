...
float4 var_U = var_M * shadowmapScale.xyxy + shadowmapScale.zzzw;
	...
	var_B.w = dot(fogSunDir, var_E);
	var_B.w = var_B.w + (-fogSunConsts.y);
	var_B.w = saturate(var_B.w * fogSunConsts.z);
...
float4 var_W = var_V * inputVx.color;
	...
	var_W.xyz *= var_V.xyz;
	var_V.x = var_V.w * (-inputVx.color.a) + kA.x;
	var_W.w = (-var_V.x) + kA.x;
	var_V.xyz = fogColorLinear.xyz;
	var_V.xyz += fogSunColorLinear.xyz;
	var_V.xyz = var_B.www * var_V.xyz + fogColorLinear.xyz;
	var_V.xyz = var_W.www * var_V.xyz;
