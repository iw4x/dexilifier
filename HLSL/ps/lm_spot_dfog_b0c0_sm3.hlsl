...
float4 var_lightmapsecondaryA = tex2D(lightmapSamplerSecondary, var_C.ba);
	...
	var_C.a = dot(var_C.rgb, lightSpotDir.xyz);
	var_C.a = saturate(var_C.a * lightSpotFactors.x + lightSpotFactors.y);
	var_lightmapsecondaryA.x = pow(abs(var_C.a), abs(lightSpotFactors.z));
	var_C.a = ((-var_C.a) >= 0 ? 0 : var_lightmapsecondaryA.x);
...
half3 var_lightmapsecondaryB = (tex2D(lightmapSamplerSecondary, var_lightmapsecondaryA.xy)).xyz;
	...
	var_lightmapsecondaryB = var_C.aaa * var_lightmapsecondaryB;
...
half4 var_lightmapprimary = tex2D(lightmapSamplerPrimary, inputVx.texcoord.zw);
	...
	var_lightmapprimary.xyz = normalize(inputVx.texcoord1.xyz);
	var_lightmapsecondary.a = saturate(dot(var_C.rgb, var_lightmapprimary.xyz));
