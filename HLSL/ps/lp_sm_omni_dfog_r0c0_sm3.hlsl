...
float4 var_outr = (var_L >= 0 ? 1 : 0);
	...
	var_outr.yzw = lightPosition.xxy + inputVx.texcoord5.xxy;
	var_shadowmapspotA.w = dot(var_outr.yzw, var_outr.yzw);
	var_shadowmapspotA.w = rsqrt(var_shadowmapspotA.w);
	var_outr.yzw = var_outr.xyz * var_shadowmapspotA.w;
...
half var_O = dot(var_outr.yzw, lightSpotDir.xyz);
	...
	var_attenuation = tex2D(attenuationSampler, var_shadowmapspotA.y);
	var_shadowmapspotA.yzw = var_attenuation.rrg * var_attenuation.rrg;
	var_attenuation.rgb = var_outr.y * lightDiffuse.xyz;
...
half4 var_modellighting = tex3D(modelLightingSampler, var_outr.yzw);
	var_shadowmapspotA.x = lerp(var_outr.z, var_outr.x, var_modellighting.w);
	var_outr.xzw = var_modellighting.xyy + var_modellighting.xyy;
	var_outr.xzw = var_outr.xyz * var_outr.xyz;
