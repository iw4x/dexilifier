...
float4 var_P = (var_O >= 0 ? float4(1, 1, 1, 1) : float4(0, 0, 0, 0));
	var_outr.y = dot(var_P, float4(0.25, 0.25, 0.25, 0.25));
	...
	var_outrA.rg = saturate((-var_shadowmapsunB.xy) + float2(8, 8));
...
half4 var_modellighting = tex3D(modelLightingSampler, var_P.xyz);
	var_shadowmapsunB.x = lerp(var_outr.y, var_modellighting.w, var_outrA.g);
...
float4 var_G = var_shadowmapsunG + (-inputVx.texcoord4.zzzz);
	...
	var_modellighting.w = lerp(var_outr.x, var_shadowmapsunB.x, var_outr.y);
	...
	var_modellighting.xyz = var_outr.www * lightDiffuse.xyz;
