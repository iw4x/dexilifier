...
var_shadowmapspotA.w = dot(var_outr.gba, var_outr.gba);
	...
	var_outr.gba = var_outr.gba * var_shadowmapspotA.www;
...
half4 var_attenuation = tex2D(attenuationSampler, var_shadowmapspotA.yy);
	var_shadowmapspotA.yzw = var_attenuation.xyz * var_attenuation.xyz;
	var_attenuation.xyz = var_outr.ggg * lightDiffuse.xyz;
...
float4 var_L = var_shadowmapspotC + (-inputVx.texcoord4.zzzz);
	...
	var_shadowmapspotA.x = lerp(var_outr.r, var_modellighting.w, var_outr.b);
	var_outr.rba = var_modellighting.xyz + var_modellighting.xyz;
	var_outr.rba = var_outr.rba * var_outr.rba;
