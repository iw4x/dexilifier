...
var_modellighting.xyz = lightPosition.xyz + (-inputVx.texcoord5);
	...
	var_modellighting.xyz = var_modellighting.xyz * var_outr.www;
	...
	var_attenuation = tex2D(attenuationSampler, var_outr.ww);
	var_attenuation.rgb = var_attenuation.rgb * var_attenuation.rgb;
	var_attenuation.rgb = var_shadowmapspotA.yyy * var_attenuation.rgb;
...
float4 var_L = var_shadowmapspotC + (-inputVx.texcoord4.zzzz);
	...
	var_shadowmapspotA.w = lerp(var_outr.x, var_modellighting.w, lightSpotFactors.w);
	var_shadowmapspotA.yzw = var_shadowmapspotA.www * var_attenuation.rgb;
	var_attenuation.rgb = var_shadowmapspotA.xxx * lightDiffuse.xyz;
