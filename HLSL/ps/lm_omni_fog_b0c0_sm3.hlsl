...
float4 var_lightmapsecondaryA = tex2D(lightmapSamplerSecondary, var_C.zw);
	...
	var_lightmapsecondary.a = dot(var_C.xyz, var_C.xyz);
	var_lightmapsecondary.a = rsqrt(var_lightmapsecondary.a);
	var_C.xyz = var_C.xyz * var_lightmapsecondary.aaa;
	var_lightmapsecondary.a = 1 / var_lightmapsecondary.a;
	var_lightmapsecondaryA.xyz = normalize(inputVx.texcoord1.xyz);
	var_C.x = saturate(dot(var_C.xyz, var_lightmapsecondaryA.xyz));
	var_C.xyz *= lightDiffuse.xyz;
...
half var_lightmapprimary = (tex2D(lightmapSamplerPrimary, inputVx.texcoord.zw)).x;
	...
	var_lightmapsecondary.rgb = var_lightmapsecondaryB * var_C.xyz + var_lightmapsecondary.rgb;
...
half4 var_H = var_colormap * inputVx.color;
	...
	var_colormap.rgb = var_H.rgb * var_H.rgb;
	var_colormap.rgb = var_H.aaa * var_colormap.rgb;
	var_colormap.a = (-var_lightmapsecondary.a) + 1;
	var_H.rgb = var_colormap.aaa * fogColorLinear.xyz;
	var_lightmapsecondary.rgb = var_colormap.rgb * var_lightmapsecondary.rgb + (-var_H.rgb);
	var_lightmapsecondary.rgb = inputVx.texcoord1.w * var_lightmapsecondary.rgb + var_H.rgb;
