...
float4 var_L = var_shadowmapspotC + inputVx.texcoord4.z;
	...
	var_outr.yzw = var_shadowmapspotC.xxy * var_outr.y + inputVx.texcoord6.xxy;
	var_modellighting = tex3D(modelLightingSampler, var_outr.yzw);
	var_outr.w = dot(var_modellighting.xyz, var_modellighting.xyz);
	var_outr.w = rsqrt(var_outr.w);
	var_modellighting.xyz = var_modellighting.xyz * var_outr.w;
...
half4 var_colormap = tex2D(colorMapSampler, inputVx.texcoord);
	...
	var_outr = (var_L >= 0 ? 1 : 0);
	var_outr.x = dot(var_outr.x, 0.25);
	var_shadowmapspotA.w = lerp(lightSpotFactors.w, var_outr.x, var_modellighting.w);
	...
	var_attenuation.rgb = var_attenuation.rgb * var_attenuation.rgb;
	var_attenuation.rgb = var_shadowmapspotA.y * var_attenuation.rgb;
	var_shadowmapspotA.yzw = var_shadowmapspotA.w * var_attenuation.rrg;
	var_attenuation.rgb = var_shadowmapspotA.x * lightDiffuse.xyz;
	var_outr.xyz = var_modellighting.xyz + var_modellighting.xyz;
	var_outr.xyz = var_outr.xyz * var_outr.xyz;
	var_outr.xyz = var_shadowmapspotA.yzw * var_attenuation.rgb + var_outr.xyz;
