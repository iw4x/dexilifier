...
float4 var_lightmapprimary = tex2D(lightmapSamplerPrimary, inputVx.texcoord.zw);
	var_shadowmapspot.x = lerp(var_outr.r, var_lightmapprimary.x, lightSpotFactors.w);
	...
	var_lightmapprimary.x = dot(var_outr.rgb, lightSpotDir.xyz);
	...
	var_lightmapprimary.yz = var_outr.a * lightFalloffPlacement.xy + lightFalloffPlacement.zw;
...
half4 var_lightmapsecondaryB = tex2D(lightmapSamplerSecondary, var_lightmapsecondary.zw);
	...
	var_outr.rgb = var_lightmapprimary.xyz * var_outr.rgb + var_lightmapsecondaryA.xyz;
...
half4 var_S = var_colormap * inputVx.color;
	...
	var_colormap.rgb = var_S.rgb * var_S.rgb;
	var_colormap.rgb = var_S.aaa * var_colormap.rgb;
	var_colormap.a = (-var_outr.a) + 1;
	var_S.rgb = var_colormap.aaa * fogColorLinear.xyz;
	var_outr.rgb = var_colormap.rgb * var_outr.rgb + (-var_S.rgb);
	var_outr.rgb = inputVx.texcoord1.w * var_outr.rgb + var_S.rgb;
